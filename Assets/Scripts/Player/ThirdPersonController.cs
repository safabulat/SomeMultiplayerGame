    ...
    public class ThirdPersonController : NetworkBehaviour
    {
        ...
        public override void OnNetworkSpawn()
        {
            ...
            if(IsClient && IsOwner)
            {
                ...
                if (cinemachineVirtual != null)
                {
                    ...
                    //cinemachineVirtual.Follow = CinemachineCameraTarget.transform;
                }
            }
            GameManager gm = GameObject.Find("GameManager").GetComponent<GameManager>();
            if(gm == null) { Debug.LogError("GM NULL"); }
            else { gm.AddActivePlayer(); }
        } 
        ...
        private void Move()
        {
            ...
            if (_input.move != Vector2.zero)
            {
                ...
                //// Determine the rotation value based on the movement direction
                //float rotation = 0.0f;
                //if (inputDirection.x > 0) // Moving forward
                //{
                //    rotation = 90.0f;
                //}
                //else if (inputDirection.x < 0) // Moving backward
                //{
                //    rotation = -90.0f;
                //}
                ...
                float smoothRotation = Mathf.SmoothDampAngle(transform.eulerAngles.y, _targetRotation, ref _rotationVelocity,
                    RotationSmoothTime);
